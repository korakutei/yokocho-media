export type DigestItem = {
  id: string;
  /**
   * このニュース自体のリリース日（発表・掲載日、"MM.DD"形式、例: "09.01"）。
   * イベントの開催日ではなく、プレスリリース/記事が公開された日を指す
   * （開催日はstartDate/endDateで別途管理する）。ダイジェスト一覧は
   * この値の降順（新しいリリースが上）で並べ替えられる。
   * パース不能な自由記述（例: "開催中"）の場合はstartDateがあればそちらで代用する。
   */
  date: string;
