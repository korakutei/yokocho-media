...
- date: そのニュース自体のリリース日("MM.DD" 形式、例: "09.01")。プレスリリース/
  記事が公開・配信された日を指し、紹介するイベントの開催日ではない(開催日が
  別にある場合でも、dateには報道日を入れる)。ダイジェスト一覧はこの値の降順
  (新しいリリースが上)で並べ替えられるため、必ず実際の配信日時を確認して
  記載すること。パース不能な自由記述(例: "開催中")は開催日が不明確な場合の
  最終手段とし、通常は配信日を優先する。
